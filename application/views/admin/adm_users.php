<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
</head>
<body>
<form>



    <h1>Программа для администриования сайта по меду</h1>


    <h3><a href="/admin/page_users">Таблица пользователей</a></h3>
    <h3><a href="/admin/t_configure">Конфигурация сайта</a></h3>
    <h3><a href="/admin/t_price_med">Цена на продукцию</a></h3>
    <h3><a href="/admin/t_comment">Отзывы пользователей о продукции</a></h3>
    <h3><a href="/admin/t_content_inf">Статьи о меде и пчелах</a></h3>





</body>
</html>